...
class CheckInTest extends NerdNookTest{
	...
	/**
	 * a profile's reputation count, represented as an int.
	 * @var profileRep
	 */
...
//		create a new CheckIn and insert into mySQL
	...
	public function testGetValidCheckInByCheckInEventId() : void {
		...
		$this->assertEquals($pdoCheckIn->getCheckInProfileId(), $this->profile->getProfileId());
		//format the date to seconds since the beginning of time to avoid round off error
		$this->assertEquals($pdoCheckIn->getCheckInDateTime(), $this->VALID_DATE);
		$this->assertEquals($pdoCheckIn->getCheckInRep(), $this->VALID_REP);
	}
	...
	public function testGetValidCheckInByCheckInProfileId() : void {
		...
		//create a new CheckIn and insert into mySQL
		$checkIn = new CheckIn($this->event->getEventId(),$this->profile->getProfileId(),
			$this->VALID_DATE, $this->VALID_REP);
		$checkIn->insert($this->getPDO());

		//grab the data from mySQL and enforce the fields match our expectations
		$results = CheckIn::getCheckInByCheckInProfileId($this->getPDO(), $checkIn->getCheckInProfileId());
		$this->assertEquals($numRows + 1, $this->getConnection()->getRowCount("checkIn"));
		$this->assertCount(1, $results);
		$this->assertContainsOnlyInstancesOf("NerdCore\NerdNook\CheckIn", $results);
	...
	public function testGetValidProfileRepByCheckInProfileId(): void {
		...
		$checkIn1 = new CheckIn($this->event->getEventId(), $this->profile->getProfileId(), $this->VALID_DATE, 2);
		$checkIn2 = new CheckIn($this->event->getEventId(), $this->profile->getProfileId(), $this->VALID_DATE, 2);
		$checkIn->insert($this->getPDO());
		$checkIn1->insert($this->getPDO());
		$checkIn2->insert($this->getPDO());

		//grab the data from mySQL and enforce the fields match our expectations
		$profileRep = CheckIn::getProfileRepByProfileId($this->getPDO(), $checkIn->getCheckInProfileId());
		$this->assertEquals($profileRep, 5);

	}


	/**
	 * test getting an invalid profile Reputation from the Profile Id
	 */
	...
	public function testGetInvalidProfileRepByCheckInProfileId(): void {
		...
		$this->assertEquals($profileRep, 0);
	}
}